...
for (const testName of readdirSync(__dirname)) {
  ...
  test(testName, async () => {
    ...
    res.diffSet.forEach((entry) => {
      ...
      // when comparing React minification, mangling may rearrange some single-char variables.
      if (entry.name1 === 'react.js') {
        return assert.strictEqual(f1.length, f2.length);
      }
